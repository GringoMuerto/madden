...
**Checked in Cowork, 2026-09-24.** Three protections had been confirmed only in Claude Code on the Mac. (1) **Claude's file tools asking before editing Madden's folder does not hold in Cowork.** Asked to create `Apps/madden/cowork-test.txt`, a Cowork session created it without asking; the Mac's `~/.claude/settings.json` does not govern Cowork. The engine's check 1 refused the next run, naming the file (`here but not on GitHub: cowork-test.txt`), and passed again once it was deleted. So a Cowork edit cannot produce picks; it can only stop them until it is removed. (2) **`--cache` is refused in both.** It no longer keys on `CLAUDECODE=1`, which Cowork may not set, but on `MADDEN_DEBUG_CACHE=1`, set only for deliberate debugging. (3) **No shell while reporting does not apply in Cowork, by design:** the Cowork copy runs the engine itself, so it holds a shell. In Cowork, the engine's own checks are what stand behind a board.
...
| Produce a submittable board from cached forecasts | `--cache` refused unless `MADDEN_DEBUG_CACHE=1`, in Claude Code and Cowork alike |
| Write any engine file with Claude's file tools | `ask` on `Edit(//Users/gringomuerto/Library/CloudStorage/GoogleDrive-cseustace@gmail.com/My Drive/Scott's Second Brain/Apps/madden/**)` and its `~/` form in `~/.claude/settings.json` (the `~/dev/madden` rules until 2026-09-24), from any Claude Code session. Verified live 2026-09-11 (history below). Does not hold in Cowork (checked 2026-09-24); there check 1 refuses any changed or extra file instead |
...
| Engine check fails | Code here differs from GitHub's main (a file changed, missing or extra); an input file outside the repo or not on GitHub; GitHub unreachable and never checked; a sheet outside `MADDEN_SHEETS_DIR`; a host the run needs refused by the network; every game kicked off; or `--cache` without `MADDEN_DEBUG_CACHE=1` | The refusal, one plain sentence naming the problem (exit 3) | Halt | No picks |
